...
def build_condition(df: pd.DataFrame, condition: dict) -> str:
    ...
    column_char = condition['column_char'] if 'column_char' in condition else None
    ...
    if operator in ['equals', 'not_equals', 'greater_than', 'less_than', 'greater_than_equal', 'less_than_equal']:
        ...
        try:
            ...
            return f"{'~' if operator == 'not_in_list' else ''}`{column}`.isin({values})"
        except ValueError:
...
def on_list_type_changed():
    if st.session_state.get("list_type") == "from_masters":
...
def build_query():
    ...
    with col3:
        ...
        if new['operator'] in ['is_null', 'not_null']:
            ...
            with character_placeholder.container():
                new['column_char'] = st.text_input("Character(s)", key="nv_char")
    ...
    with col4:
        ...
        if st.session_state.list_source_str and list_type == "from_masters":
    ...
    with col1:
        if st.button(f"Add", key="add_query_condition", icon=":material/add:"):            
            if (not new['column'] or not new['operator'] or not new['logical']):
    ...
    if st.session_state.validation['conditions']:
        ...
        for i, cond in enumerate(st.session_state.validation['conditions']):
            ...
            with col4:
                st.write(f"{cond.get('logical', 'AND')}")
            with col5:
